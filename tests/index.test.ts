...
import { APIPromise } from 'testing-mcp-openapi/core/api-promise';

import util from 'node:util';
import TestingMcpOpenAPI from 'testing-mcp-openapi';
import { APIUserAbortError } from 'testing-mcp-openapi';
...
describe('logging', () => {
  ...
  beforeEach(() => {
    process.env = { ...env };
    process.env['TESTING_MCP_OPENAPI_LOG'] = undefined;
  });
  ...
  const forceAPIResponseForClient = async (client: TestingMcpOpenAPI) => {
  ...
  test('default logLevel is warn', async () => {
    const client = new TestingMcpOpenAPI({ apiKey: 'My API Key' });
  ...
  test('debug logs happen with debug env var', async () => {
    ...
    process.env['TESTING_MCP_OPENAPI_LOG'] = 'debug';
    const client = new TestingMcpOpenAPI({ logger: logger, apiKey: 'My API Key' });
  ...
  test('warn when env var level is invalid', async () => {
    ...
    process.env['TESTING_MCP_OPENAPI_LOG'] = 'not a log level';
    const client = new TestingMcpOpenAPI({ logger: logger, apiKey: 'My API Key' });
    expect(client.logLevel).toBe('warn');
    expect(warnMock).toHaveBeenCalledWith('process.env[\'TESTING_MCP_OPENAPI_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]');
  });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new TestingMcpOpenAPI({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['TESTING_MCP_OPENAPI_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new TestingMcpOpenAPI({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['TESTING_MCP_OPENAPI_BASE_URL'] = 'https://example.com/from_env';
      const client = new TestingMcpOpenAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['TESTING_MCP_OPENAPI_BASE_URL'] = ''; // empty
      const client = new TestingMcpOpenAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://My-Environment.petstore.io')
    });

    test('blank env variable', () => {
      process.env['TESTING_MCP_OPENAPI_BASE_URL'] = '  '; // blank
      const client = new TestingMcpOpenAPI({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://My-Environment.petstore.io')
    });
  ...
  test('maxRetries option is correctly set', () => {
    const client = new TestingMcpOpenAPI({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new TestingMcpOpenAPI({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['TESTING_MCP_OPENAPI_API_KEY'] = 'My API Key';
    const client = new TestingMcpOpenAPI();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['TESTING_MCP_OPENAPI_API_KEY'] = 'another My API Key';
    const client = new TestingMcpOpenAPI({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new TestingMcpOpenAPI({ apiKey: 'My API Key', fetch: testFetch });
