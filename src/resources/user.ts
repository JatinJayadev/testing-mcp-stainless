...
export class UserResource extends APIResource {
   ...
   * @example
   * ```ts
   * const user = await client.user.delete('username');
   * ```
   */
  delete(username: string, options?: RequestOptions): APIPromise<User> {
    return this._client.delete(path`/user/${username}`, options);
  }
