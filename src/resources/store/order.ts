...
export class OrderResource extends APIResource {
   ...
   * @example
   * ```ts
   * const order = await client.store.order.delete(0);
   * ```
   */
  delete(orderID: number, options?: RequestOptions): APIPromise<Order> {
    return this._client.delete(path`/store/order/${orderID}`, options);
  }
}
