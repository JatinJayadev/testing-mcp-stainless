...
import { Pet, PetCreateParams, PetFindByStatusParams, PetFindByStatusResponse, PetFindByTagsParams, PetFindByTagsResponse, PetResource, PetUpdateParams, PetUploadImageParams, PetUploadImageResponse } from './resources/pet';
...
export interface ClientOptions {
  /**
   * Defaults to process.env['TESTING_MCP_OPENAPI_API_KEY'].
   */
   ...
   *
   * Defaults to process.env['TESTING_MCP_OPENAPI_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['TESTING_MCP_OPENAPI_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Testing Mcp OpenAPI API. 
 */
export class TestingMcpOpenAPI {
  apiKey: string;
  environment: string;
   ...
   * API Client for interfacing with the Testing Mcp OpenAPI API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['TESTING_MCP_OPENAPI_API_KEY'] ?? undefined]
   * @param {string | undefined} [opts.environment=process.env['TESTING_MCP_OPENAPI_ENVIRONMENT'] ?? prod]
   * @param {string} [opts.baseURL=process.env['TESTING_MCP_OPENAPI_BASE_URL'] ?? https://{environment}.petstore.io] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('TESTING_MCP_OPENAPI_BASE_URL'),
    apiKey = readEnv('TESTING_MCP_OPENAPI_API_KEY'),
    environment = readEnv('TESTING_MCP_OPENAPI_ENVIRONMENT') ?? 'prod',
    ...opts
  }: ClientOptions = {}) {
    if (apiKey === undefined) {
      throw new Errors.TestingMcpOpenAPIError(
        'The TESTING_MCP_OPENAPI_API_KEY environment variable is missing or empty; either provide it, or instantiate the TestingMcpOpenAPI client with an apiKey option, like new TestingMcpOpenAPI({ apiKey: \'My API Key\' }).'
      );
    }
    ...
    const options: ClientOptions = {
      ...
      environment,
      ...opts,
      baseURL: baseURL || `https://${environment}.petstore.io`,
    };

    this.baseURL = options.baseURL!;
    this.timeout = options.timeout ?? TestingMcpOpenAPI.DEFAULT_TIMEOUT /* 1 minute */;
    this.logger = options.logger ?? console;
    const defaultLogLevel = 'warn';
    // Set default logLevel early so that we can log a warning in parseLogLevel.
    this.logLevel = defaultLogLevel;
    this.logLevel = parseLogLevel(options.logLevel, 'ClientOptions.logLevel', this) ?? parseLogLevel(readEnv('TESTING_MCP_OPENAPI_LOG'), 'process.env[\'TESTING_MCP_OPENAPI_LOG\']', this) ?? defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://{environment}.petstore.io';
  }
  ...
  static TestingMcpOpenAPI = this;
  static DEFAULT_TIMEOUT = 60000 // 1 minute

  static TestingMcpOpenAPIError = Errors.TestingMcpOpenAPIError;
...
TestingMcpOpenAPI.PetResource = PetResource;
TestingMcpOpenAPI.Store = Store;
TestingMcpOpenAPI.UserResource = UserResource;

export declare namespace TestingMcpOpenAPI {
